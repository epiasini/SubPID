function [I_shar,I_syn,I_unx,I_uny,q_opt]=PID_code_line_search(p,accuracy,method,lin_accuracy)
...
%                 % internal
    ...
    if (iter>1 && (dot(deriv,coeff_prev-coeff_tot)<=accuracy) ) || max(abs(coeff_prev))<=accuracy

        check=1; %exit the algorithm
        q_opt=q; % output the optimal distribution
    else
        
     %line-search increment      
