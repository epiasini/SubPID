...
while aa<=ceil(1/lambdastep)+1 % loop over different correlations between the dice
   ...
   for xx=1:dimx
    ...
    [I_shar(aa),I_syn(aa),I_unx(aa),I_uny(aa),q_1]=PID_code(dimz,dimy,dimx,p,accuracy,'glpk',0);
...
toc
   ...
   [I_shar_z(aa),I_syn_z(aa),I_unx_z(aa),I_uny_z(aa),q_3]=PID_code(dimz_z,dimy_z,dimx_z,p_z,accuracy,'glpk',0.);
